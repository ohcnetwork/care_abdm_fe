...
const DateInputV2: React.FC<Props> = ({
  ...
  const getPosition = () => {
    ...
    return `${right ? "md:-translate-x-1/2" : ""} ${
      top ? "md:-translate-y-[calc(100%+50px)]" : ""
    }`;
  };
              ...
              <div>
                ...
                >
                  ...
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 p-2">
                    <CalendarIcon className="text-lg text-secondary-600" />
                  </div>
                </PopoverButton>
                {open && (
                  <PopoverPanel
                    className={cn(
                      `cui-dropdown-base absolute my-0.5 ${
                        allowTime
                          ? "max-h-[80vh] w-full md:h-auto md:w-[400px]"
                          : "w-72"
                      } divide-y-0 rounded p-4`,
                      getPosition(),
                      popOverClassName
                    )}
                  >
