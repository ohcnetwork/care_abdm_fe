  ...
  return (
    <>
      <Dialog
        open={
          !abhaNumber &&
          qParams["show_link_abha_number"] !== "true" &&
          qParams["show_expired_discharge_form"] !== "true"
        }
        onOpenChange={() => {}}
      >
        <DialogContent className="sm:max-w-lg [&>button]:hidden">
          ...
          <div className="flex items-center justify-end gap-2 mt-6 flex-col w-full">
            ...
            <div
              className="bg-red-600 pointer-events-auto w-full flex cursor-pointer items-center justify-center gap-2 rounded border-0 p-2 text-sm font-normal transition-all duration-200 ease-in-out"
              onClick={() => {
                close();
                setQParams({
                  ...qParams,
                  show_expired_discharge_form: "true",
                });
              }}
            >
              <span className="flex w-full items-center justify-center gap-2 text-center text-white">
                <CareIcon icon="l-hospital" className="text-lg" />
                <p>Discharge as Expired</p>
              </span>
            </div>
      ...
      {consultation && (
        <DischargeModal
          show={qParams["show_expired_discharge_form"] === "true"}
          onClose={() => {
            setQParams({
              ...qParams,
              show_expired_discharge_form: undefined,
            });
          }}
          consultationData={consultation}
          new_discharge_reason={3}
        />
      )}
    </>
  );
